...
import { ConversationsService, PlainConversationWithUser } from './conversations.service';
import { UseGuards } from '@nestjs/common';
import { AuthGuard } from '../common/guards/auth.guard';
import { TenantGuard } from '../common/guards/tenant.guard';

@Controller(':tenantId/conversations')
...
export class ConversationsController {
  ...
  ): Promise<{ conversations: PlainConversationWithUser[]; lastUpdated: string }> {
    try {
      const conversationLimit = limit ? parseInt(limit, 10) : 50;
      const conversations = await this.conversationsService.getConversationsWithUsers(
        tenantId,
        conversationLimit
      );
  ...
  async streamConversations(
    ...
    try {
      const stream = this.conversationsService.createSseStream(tenantId, request);
  ...
  async createThreadReply(
    ...
    const { parentConversationId, messageText, messageTs, userId, channelId } = body;
